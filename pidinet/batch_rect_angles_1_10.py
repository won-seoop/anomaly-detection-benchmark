...
OUT_DIR = TASK_ROOT / "result" / "angles" / "2" / "pidinet"
...
def pidinet_mask_and_contour(model, img):
    ...
    if not contours:
        return None, binary, offset
    return max(contours, key=cv2.contourArea), binary, offset
...
def main():
    ...
    for path in sorted(DATA_DIR.glob("*.png"), key=lambda p: int(p.stem)):
        img = imread(path)
        contour, binary, (ox, oy) = pidinet_mask_and_contour(model, img)
        out = img.copy()
        ...
        min_angle, min_rect = min_area_angle(contour_full)
        ...
        if best is not None:
            ...
            pts += np.array([ox, oy], dtype=np.float32)
        ...
        row.update({
            ...
            "contour_area": f"{cv2.contourArea(contour_full):.1f}",
        })
        rows.append(row)
        print(row)
